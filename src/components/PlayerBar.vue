  ...
  <div class="player-wrap" @click="setPlayerShow(true)" v-show="show">
    ...
    <div class="btn-wrap">
      <i
        ...
        @click.stop="play"
      ></i>
      <i class="iconfont btn btn-list" @click.stop="setPlaylistShow(true)"
        >&#xe604;</i
      >
    </div>
    <!-- 播放列表 -->
...
export default {
  ...
  //有点没理解，备注：watch监听playstate状态改变图片旋转方式
  watch: {
  ...
  mounted: function() {
    window.onresize = () => {
      if (window.innerHeight <= 320 || document.body.clientHeight <= 320) {
        // 页面高度小于320时，隐藏底部播放工具
        this.show = false;
      } else {
        this.show = true;
