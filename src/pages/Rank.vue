  ...
  <div class="container" v-if="rank">
    ...
    <div class="header">
      <div
        id="background"
        class="background"
        :style="{ backgroundImage: 'url(' + rank.topinfo.pic_album + ')' }"
      >
        <div class="mask"></div>
      </div>
      <!-- 顶部的下方的排行榜信息 -->
      <div class="rank-info">
        <p class="rank-name">
          {{ rank.topinfo.ListName }} 第{{ rank.day_of_year }}天
        </p>
  ...
  methods: {
    ...
    contentScroll: function() {
      ...
      if (this.scroll === 0 && top > this.scroll) {
        document.getElementById("fixedHeader").style.backgroundColor =
          "transparent";
        document.getElementById("background").style.filter = "none";
        this.showFixedTitle = false;
      } else if (top > this.scroll) {
        // 上滚
      } else if (top < this.scroll) {
        // 下滚
      }
